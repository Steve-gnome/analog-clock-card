...
class AnalogClockCard extends HTMLElement {
  ...
  static getStubConfig() { return { size: 220 }; }
  ...
  _getNow() {
    ...
    const timeStr = `${pad(h)}:${pad(m)}`;  // HH:MM only
    const dateFmt = new Intl.DateTimeFormat(this._locale, {
      timeZone: tz, weekday: 'short', day: 'numeric', month: 'short',
    });
    return { h, m, s, timeStr, dateStr: dateFmt.format(now) };
  }
  ...
  _draw() {
    ...
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2);
    ctx.strokeStyle = c.border; ctx.lineWidth = Math.max(1, r * 0.022); ctx.stroke();

    for (let i = 0; i < 60; i++) {
      const angle   = (i * Math.PI * 2) / 60 - Math.PI / 2;
      const isMajor = i % 5 === 0;
      ctx.beginPath();
      ctx.moveTo(cx + Math.cos(angle) * (isMajor ? r * 0.80 : r * 0.88), cy + Math.sin(angle) * (isMajor ? r * 0.80 : r * 0.88));
      ...
      ctx.lineWidth   = isMajor ? Math.max(1.5, r * 0.025) : Math.max(0.8, r * 0.012);
      ctx.stroke();
    }

    const numR = r * 0.68;
    const fs   = Math.max(10, Math.round(r * 0.18));
    ...
    for (let n = 1; n <= 12; n++) {
      ...
      ctx.fillText(String(n), cx + Math.cos(a) * numR, cy + Math.sin(a) * numR);
    }

    const { h, m, s, timeStr, dateStr } = this._getNow();

    // digital time — halfway between 12 and centre
    const ds = Math.max(9, Math.round(r * 0.175));
    ctx.font = `400 ${ds}px "SF Mono", "Consolas", monospace`;
    ctx.fillStyle = c.digitalTime; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(timeStr, cx, cy - r * 0.28);

    // date — lower centre
    const dts = Math.max(8, Math.round(r * 0.155));
    ctx.font = `500 ${dts}px -apple-system, "Helvetica Neue", Arial, sans-serif`;
    ctx.fillStyle = c.date; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(dateStr, cx, cy + r * 0.42);
    ...
    this._drawHand(ctx, cx, cy, hourAngle, r * 0.48, r * 0.032, c.hourHand,   r * 0.12);
    this._drawHand(ctx, cx, cy, minAngle,  r * 0.70, r * 0.022, c.minuteHand, r * 0.12);
    this._drawSecondHand(ctx, cx, cy, secAngle, r, c.secondHand);
  ...
  _drawHand(ctx, cx, cy, angle, length, width, color, tail) {
    ...
    ctx.moveTo(-tail, 0); ctx.lineTo(length, 0); ctx.stroke();
    ctx.restore();
  }

  _drawSecondHand(ctx, cx, cy, angle, r, color) {
    ctx.save(); ctx.translate(cx, cy); ctx.rotate(angle);
    ctx.strokeStyle = color; ctx.lineCap = 'round';
    ctx.beginPath(); ctx.lineWidth = Math.max(1, r * 0.014);
    ctx.moveTo(-r * 0.18, 0); ctx.lineTo(r * 0.82, 0); ctx.stroke();
    ctx.beginPath(); ctx.arc(-r * 0.12, 0, r * 0.03, 0, Math.PI * 2);
...
window.customCards.push({
  type: 'analog-clock-card', name: 'Analog Clock Card',
  description: 'Clean analog clock with digital time and date. Cross-platform safe.',
  preview: false,
});
console.info('%c ANALOG-CLOCK-CARD v2.5 ', 'color: white; font-weight: bold; background: #1a1a2e');
